...
def process_file(context: ProcessorContext, input: os.PathLike):
    ...
    with open(input, encoding="utf-8") as f:
        ...
        content = context.block_comment_pattern.sub(lambda m: replace_copyright(context, m), content)
...
def parse_args():
    parser = ArgumentParser(prog="amalgamate",
                                     description="Combines a C++ source/header hierarchy into one file")
