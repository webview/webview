...
inline std::map<uint32_t, JsCallback *> cbMap{};
...
extern "C" {
     ...
     * The `arg` parameter is used to piggy-back the `JsCallback` instance UID along with the `arg` ID.
     */
    class Trampoline {
        public:
            ...
            using tsfn_t = TypedThreadSafeFunction<
                tsfnContext,
                tsfnData_t,
                Trampoline::callTsfn>;
        ...
        private:
            ...
            static tsfnData_t *makeTsfnData(
                std::string id,
                std::string req,
                webview_t w,
                ids_t ids
 ...
 * @param {Napi::Function} callback - The user provided JS cb function.
 */
class JsCallback : public ObjectWrap<JsCallback> {
    public:
         ...
         * @return {void}
         */
        void cleanup(const CallbackInfo &info);

        /**
         * Gets a unique identifier for the callback instance.
         * @return {Napi::Number}
         */
        Napi::Value getCbUid(const CallbackInfo &info);

        /**
         * Gets a `JsCallback` instance specific ID for a user provided `arg`.
         * @return {Napi::Number}
         */
        Napi::Value getArgId(const CallbackInfo &info);
        ...
        Trampoline::tsfn_t tsfn;

        /**Tracks the lifecycle of the cb function instance */
        status_t cbStatus = opened;

        /** A flag to indicate if an instance cleanup should be a done in a thread-safe context. */
        bool tsCleanup = true;

        /** A lambda to clean up the `JsCallback` instance without mutex locking. */
        std::function<void()> cleanup_f;

        /** A lambda to clean up the `JsCallback` instance  with mutex locking. */
        std::function<void()> cleanup_tsf;
